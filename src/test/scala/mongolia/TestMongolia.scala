...
class TestMongolia {
  ...
  def nullFilter {
    ...
    dbo("bar") match {
      case f: Undefined => assertTrue(true)
  ...
  def mapReduceCoffee = compilerPool.borrow { implicit comp =>
    ...
    println("Map:\n" + mapReduce.mapJS)
    println("Reduce:\n" + mapReduce.reduceJS)
    val coffeescript = new java.io.StringReader(s"""
map  =  $map
reduce=$reduce
      """)
  ...
  def interfaces {
    ...
    Json.parseObject("""{"foo":"bar","nested":{"two":2,"three":3,"fortytwo":42,"baz":null},"definite":666,"list2":[1.0,2.0,3.0]}""").map(_.like[Foo]) match {
  ...
  def `interface mismatch` {
    ...
    try {
      ...
      case InvalidValueTypeException(name) => assertEquals("baz", name)
    }
  }
